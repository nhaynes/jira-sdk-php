...
class FindUsersForPicker extends \JiraSdk\Runtime\Client\BaseEndpoint implements \JiraSdk\Runtime\Client\Endpoint
{
    ...
    */
    ...
    protected function getQueryOptionsResolver(): \Symfony\Component\OptionsResolver\OptionsResolver
    {
        ...
        $optionsResolver->setDefined(array('query', 'maxResults', 'showAvatar', 'exclude', 'excludeAccountIds', 'avatarSize', 'excludeConnectUsers'));
        $optionsResolver->setRequired(array('query'));
        $optionsResolver->setDefaults(array('maxResults' => 50, 'showAvatar' => false, 'excludeConnectUsers' => false));
        ...
        $optionsResolver->addAllowedTypes('excludeAccountIds', array('array'));
        $optionsResolver->addAllowedTypes('avatarSize', array('string'));
        $optionsResolver->addAllowedTypes('excludeConnectUsers', array('bool'));
     ...
     *
     * @return null|\JiraSdk\Model\FoundUsers
     */
    protected function transformResponseBody(\Psr\Http\Message\ResponseInterface $response, \Symfony\Component\Serializer\SerializerInterface $serializer, ?string $contentType = null)
    {
        ...
        if (is_null($contentType) === false && (200 === $status && mb_strpos($contentType, 'application/json') !== false)) {
            return $serializer->deserialize($body, 'JiraSdk\\Model\\FoundUsers', 'json');
        }
        if (400 === $status) {
            throw new \JiraSdk\Exception\FindUsersForPickerBadRequestException($response);
        }
        if (401 === $status) {
            throw new \JiraSdk\Exception\FindUsersForPickerUnauthorizedException($response);
        }
        if (429 === $status) {
            throw new \JiraSdk\Exception\FindUsersForPickerTooManyRequestsException($response);
        }
    }
