...
class GetAllUsersDefault extends \JiraSdk\Api\Runtime\Client\BaseEndpoint implements \JiraSdk\Api\Runtime\Client\Endpoint
{
    ...
    protected function transformResponseBody(\Psr\Http\Message\ResponseInterface $response, \Symfony\Component\Serializer\SerializerInterface $serializer, ?string $contentType = null)
    {
        ...
        if (400 === $status) {
            throw new \JiraSdk\Api\Exception\GetAllUsersDefaultBadRequestException($response);
        }
        if (403 === $status) {
            throw new \JiraSdk\Api\Exception\GetAllUsersDefaultForbiddenException($response);
        }
        if (409 === $status) {
            throw new \JiraSdk\Api\Exception\GetAllUsersDefaultConflictException($response);
        }
    }
}
