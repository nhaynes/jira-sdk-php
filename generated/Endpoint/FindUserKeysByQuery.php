...
class FindUserKeysByQuery extends \JiraSdk\Api\Runtime\Client\BaseEndpoint implements \JiraSdk\Api\Runtime\Client\Endpoint
{
     ...
     * {@inheritdoc}
     *
     * @return \JiraSdk\Api\Model\PageBeanUserKey|null
     *
    ...
    protected function transformResponseBody(\Psr\Http\Message\ResponseInterface $response, \Symfony\Component\Serializer\SerializerInterface $serializer, ?string $contentType = null)
    {
        ...
        if ((null === $contentType) === false && (200 === $status && false !== mb_strpos($contentType, 'application/json'))) {
            return $serializer->deserialize($body, 'JiraSdk\\Api\\Model\\PageBeanUserKey', 'json');
        }
        if (400 === $status) {
            throw new \JiraSdk\Api\Exception\FindUserKeysByQueryBadRequestException($response);
        }
        if (401 === $status) {
            throw new \JiraSdk\Api\Exception\FindUserKeysByQueryUnauthorizedException($response);
        }
        if (403 === $status) {
            throw new \JiraSdk\Api\Exception\FindUserKeysByQueryForbiddenException($response);
        }
        if (408 === $status) {
            throw new \JiraSdk\Api\Exception\FindUserKeysByQueryRequestTimeoutException($response);
        }
    }
}
