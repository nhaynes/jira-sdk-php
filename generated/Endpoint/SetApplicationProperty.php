...
class SetApplicationProperty extends \JiraSdk\Runtime\Client\BaseEndpoint implements \JiraSdk\Runtime\Client\Endpoint
{
    ...
    */
    public function __construct(string $id, \JiraSdk\Model\SimpleApplicationPropertyBean $requestBody)
    {
        $this->id = $id;
    ...
    {
        return str_replace(array('{id}'), array($this->id), '/rest/api/3/application-properties/{id}');
    }
    public function getBody(\Symfony\Component\Serializer\SerializerInterface $serializer, $streamFactory = null): array
    {
        if ($this->body instanceof \JiraSdk\Model\SimpleApplicationPropertyBean) {
    ...
    protected function transformResponseBody(\Psr\Http\Message\ResponseInterface $response, \Symfony\Component\Serializer\SerializerInterface $serializer, ?string $contentType = null)
    {
        ...
        if (400 === $status) {
            throw new \JiraSdk\Exception\SetApplicationPropertyBadRequestException($response);
        }
        if (401 === $status) {
            throw new \JiraSdk\Exception\SetApplicationPropertyUnauthorizedException($response);
        }
        if (403 === $status) {
            throw new \JiraSdk\Exception\SetApplicationPropertyForbiddenException($response);
        }
        if (404 === $status) {
            throw new \JiraSdk\Exception\SetApplicationPropertyNotFoundException($response);
        }
    }
