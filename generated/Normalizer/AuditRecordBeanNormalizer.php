...
class AuditRecordBeanNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        return 'JiraSdk\\Api\\Model\\AuditRecordBean' === $type;
    }

    public function supportsNormalization($data, $format = null): bool
    {
        return \is_object($data) && 'JiraSdk\\Api\\Model\\AuditRecordBean' === \get_class($data);
    }
    ...
    public function denormalize($data, $class, $format = null, array $context = [])
    {
        ...
        $object = new \JiraSdk\Api\Model\AuditRecordBean();
        ...
        if (\array_key_exists('objectItem', $data)) {
            $object->setObjectItem($this->denormalizer->denormalize($data['objectItem'], 'JiraSdk\\Api\\Model\\AssociatedItemBean', 'json', $context));
        }
        if (\array_key_exists('changedValues', $data)) {
            $values = [];
            foreach ($data['changedValues'] as $value) {
                $values[] = $this->denormalizer->denormalize($value, 'JiraSdk\\Api\\Model\\ChangedValueBean', 'json', $context);
            }
        ...
        if (\array_key_exists('associatedItems', $data)) {
            ...
            foreach ($data['associatedItems'] as $value_1) {
                $values_1[] = $this->denormalizer->denormalize($value_1, 'JiraSdk\\Api\\Model\\AssociatedItemBean', 'json', $context);
            }
